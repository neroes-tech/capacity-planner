...
export default function EntryList({ week }: Props) {
  ...
  if (isLoading) {
    return <p className="text-sm text-gray-400 mt-4 animate-pulse">A carregar registos…</p>
  }

  if (isError) {
    return <p role="alert" className="text-sm text-red-600 mt-4">Erro ao carregar registos.</p>
  }
      ...
      <ul className="space-y-1" aria-label={`Registos ${title}`}>
        {entries.map((entry) => (
          ...
          >
            ...
            <button
              onClick={() => onDelete({ id: entry.id, week })}
              aria-label={`Apagar registo de ${entry.hours}h de ${entry.person.name} em ${entry.workspace.short_name}`}
